...
**Phase 5 — Inline AI (implemented, on branch `feature/inline-ai`, branched off `feature/realtime-collab`):**

- **Deliberately did not use Blocknote's official AI package** (`@blocknote/xl-ai`) — it hard-depends on `@blocknote/mantine` in its own `dependencies` (not just an example), which would have reintroduced the exact framework Phase 3 chose to avoid, for a feature that's actually narrow and fixed (4 prompts on selected text) rather than the open-ended chat/tool-calling architecture that package is built around. It's also single-model by default, so the required 3-provider fallback chain needed hand-writing regardless of frontend approach — meaning adopting it wouldn't even have saved the one genuinely custom piece.
- Hand-rolled instead: `src/lib/inlineAi.ts` tries OpenAI → Gemini → Groq in order via the Vercel AI SDK's `generateText`, catching and falling through on any failure (missing key, bad model, provider outage). Model IDs are env-configurable (`OPENAI_MODEL`, `GEMINI_MODEL`, `GROQ_MODEL`) rather than hardcoded.
- `POST /api/ai/inline` — any authenticated session (manager or employee), no page-specific check needed since it's a stateless text transform (the actual content replacement happens client-side, nothing is read or written server-side per-page).
- `src/components/InlineAiToolbar.tsx` — a small floating toolbar built directly on `BlockNoteEditor`'s own public API (`editor.getSelectedText()`, `editor.onSelectionChange()`, `editor.pasteText()`) and the standard DOM Selection API for positioning — no Blocknote extension system involved, consistent with the "avoid a heavy framework for a narrow need" pattern already used for OAuth in Phase 2.
- **Known, documented limitation**: if the user changes their selection while a request is in flight, `pasteText` replaces whatever is selected when the response arrives, not the original selection. Buttons are disabled during the request to shrink the window; not eliminated, since a full fix would mean tracking/restoring exact cursor positions.
- `npm run lint` and `npx tsc --noEmit` both clean.
- Live-tested against real Mongo: unauthenticated → 401, invalid action → 400, empty text → 400, over-length text → 400, and a valid request → a clean 502 ("AI is unavailable") since no provider API keys are in `.env.local` yet — confirmed via the server log that this is a graceful catch, not an unhandled crash. The page editor route renders correctly with the toolbar wired in.
- **Not verified here**: actual AI generation and the fallback chain genuinely falling through from one provider to the next both need at least one real API key, which isn't configured yet — same "code path verified, real credentials pending" pattern as OAuth in Phase 2. The floating toolbar's positioning/selection UX also can't be click-tested (no headless browser tooling in this sandbox).

## Next

- Push all six branches (`feature/manager-auth`, `feature/workspace-crud`, `feature/employee-invitation`, `feature/page-crud`, `feature/realtime-collab`, `feature/inline-ai`) — still blocked, no GitHub credentials in this sandbox — and open PRs.
- Add at least one of `OPENAI_API_KEY` / `GOOGLE_GENERATIVE_AI_API_KEY` / `GROQ_API_KEY` to `.env.local` so inline AI can be live-tested end-to-end (and ideally more than one, to verify the fallback actually falls through).
- **Three things still need a human with a real browser to fully close out**: the employee OAuth sign-in flow, real-time collaboration between two tabs, and the inline AI toolbar's click/selection UX.
- This closes out the current 3-phase batch (Phases 3–5). Next up per the roadmap: Phase 6 (search + recently visited).
